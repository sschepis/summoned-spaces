...
import { DatabaseAdapter } from '../lib/database/abstract-adapter.js';
 ...
 */
export async function initializeDatabase(customConfig?: Partial<DatabaseConfig>): Promise<void> {
    try {
        console.log('🚀 Initializing Summoned Spaces database...');
        
        // Determine configuration
        const config = customConfig?.type ? customConfig as DatabaseConfig : await getDefaultConfig();
 ...
 */
async function getDefaultConfig(): Promise<DatabaseConfig> {
 ...
 */
export async function initializeDatabaseForEnvironment(): Promise<void> {
    const isVercel = process.env.VERCEL === '1';
    const hasNeonUrl = !!(process.env.DATABASE_URL || process.env.NEON_DATABASE_URL);
